...
class DSelect_k(MMoE):
    ...
    def __init__(self, task_name, encoder_class, decoders, rep_grad, multi_input, device, **kwargs):
        ...
        self._z_logits = nn.ModuleDict({task: nn.Linear(3*244*244, 
                                                        self._num_nonzeros*self._num_binary) for task in self.task_name})
        
        # self._w_logits = nn.ModuleDict({task: nn.Linear(self.input_size, self._num_nonzeros) for task in self.task_name})
        
        self._w_logits = nn.ModuleDict({task: nn.Linear(3*244*244, self._num_nonzeros) for task in self.task_name})
    ...
    def forward(self, inputs, task_name=None):
        x = inputs.reshape(inputs.shape[0], -1)  # inputs: [bs, c, w, h]  --->>  x: [bs, cwh]
        experts_shared_rep = torch.stack([e(inputs) for e in self.experts_shared])
        out = {}
        for task in self.task_name:
            if task_name is not None and task != task_name:
                continue
            sample_logits = self._z_logits[task](torch.flatten(x, start_dim=1))
            ...
            selector_weights = F.softmax(self._w_logits[task](torch.flatten(x, start_dim=1)), dim=1)
