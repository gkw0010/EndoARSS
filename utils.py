...
# class
class ClassMetric(AbsMetric):
    def __init__(self, num_classes):
        super(ClassMetric, self).__init__()
...
class ClassLoss(AbsLoss):
    def __init__(self, cls_loss_w=1.0):
        super(ClassLoss, self).__init__()
        self.loss_fn = nn.CrossEntropyLoss(ignore_index=-1)
        self.loss_w = cls_loss_w

    def compute_loss(self, pred, gt):
        return self.loss_fn(pred, gt.long()) * self.loss_w
